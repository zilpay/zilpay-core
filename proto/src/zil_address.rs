use config::address::{ADDR_LEN, CHARSET, GENERATOR, HRP};
...
pub fn from_zil_bech32_address(address: &str) -> Option<[u8; ADDR_LEN]> {
...
pub fn to_zil_bech32(value: &[u8; ADDR_LEN]) -> Option<String> {
    convert_bits(value, 8, 5, true).map(|addrbz| encode(HRP, &addrbz))
}
