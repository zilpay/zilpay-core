...
mod tests_background_transactions {
    ...
    async fn test_sign_and_send_tron_trc20_tx() {
        ...
        let trx_balance = *ftokens.first().unwrap().balances.get(&0).unwrap();
        let trc20 = ftokens.get(1).unwrap();
        let trc20_balance = *trc20.balances.get(&0).unwrap();

        let account_0 = data.accounts.first().unwrap();
        let account_1 = data.accounts.get(1).unwrap();
        assert_eq!(account_0.addr.auto_format(), tron_addresses::ADDR_0);

        eprintln!("[TRC20] sender:         {}", account_0.addr.auto_format());
        eprintln!("[TRC20] recipient:       {}", account_1.addr.auto_format());
        eprintln!("[TRC20] TRX balance:     {}", trx_balance);
        eprintln!("[TRC20] TRC20 balance:   {}", trc20_balance);
        eprintln!("[TRC20] token contract:  {}", trc20.addr.auto_format());

        let mut tx_request = bg
            .build_token_transfer(trc20, account_0, account_1.addr.clone(), U256::from(1u64))
            .await
            .unwrap();
        ...
        let mut tron_tx = match tx_request {
            TransactionRequest::Tron((t, _)) => t,
        ...
        let signed = tx_request.sign(&keypair).await.unwrap();
        assert!(signed.verify().unwrap());
        eprintln!("[TRC20] tx signed and verified");
